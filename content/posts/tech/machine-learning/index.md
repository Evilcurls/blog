...
date: 2025-03-03T22:15:15+08:00
draft: false
categories: ["技术"]
tags: ["ML"]
lightgallery: true
...
- **模型 (Model)：** 用于拟合输入与输出关系的函数表达式 $f(x, \theta)$。
- **学习准则 (Learning Criterion)：** 衡量模型预测值与真实值之间差异的评价标准（通常指损失函数或目标函数）。
- **优化算法 (Optimization Algorithm)：** 用于寻找最优模型参数 $\theta$ 的具体计算方法（如梯度下降法）。

### 二、 三种主要学习算法

#### 1. 监督学习 (Supervised Learning)

利用带有明确“标签”的数据进行训练，让模型学会输入到输出的映射。根据标签数据的类型，可分为：

- **回归问题 (Regression)：** 标签 $y$ 是连续值，模型 $f(x, \theta)$ 的输出也是连续数值（例如：预测房价或气温）。
- **分类问题 (Classification)：** 标签 $y$ 是离散的类别值（例如：判断邮件是否为垃圾邮件）。
- **结构化学习问题 (Structured Learning)：** 本质上是复杂的特殊分类问题，其标签 $Y$ 不是单一的数值或类别，而是结构化的对象，如**序列、树或图**（例如：句子解析、机器翻译）。

#### 2. 无监督学习 (Unsupervised Learning)

训练样本**不包含目标标签**，模型需要自主从数据中探索和学习有价值的内部结构信息。常见任务包括：

- **聚类 (Clustering)：** 将特征相似的样本自动划分到不同的簇中。
- **密度估计 (Density Estimation)：** 估计数据在特征空间中的概率分布。
- **特征学习与降维 (Feature Learning & Dimensionality Reduction)：** 提取数据的核心特征，去除冗余信息并降低维度。

#### 3. 强化学习 (Reinforcement Learning)

一种**基于交互**的机器学习算法。它不依赖给定的标签数据，而是智能体（Agent）在与环境（Environment）的不断交互（试错）中进行学习，根据环境给出的反馈（奖励或惩
...
![](https://raw.githubusercontent.com/Evilcurls/image4blog/main/一元模型三要素.png)
...
![](https://raw.githubusercontent.com/Evilcurls/image4blog/main/sigmoid函数.png)
...
sigmoid函数=$ \frac{1} {1+e^{-x}} $
 ...
 ![](https://raw.githubusercontent.com/Evilcurls/image4blog/main/二分类问题.png)
...
![](https://raw.githubusercontent.com/Evilcurls/image4blog/main/logistic%26tanh.png)
...
![](https://raw.githubusercontent.com/Evilcurls/image4blog/main/前馈神经网络.png)
...
![](https://raw.githubusercontent.com/Evilcurls/image4blog/main/卷积作用图.png)
